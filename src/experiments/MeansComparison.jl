# MeansComparison.jl
#
# The same three reference measures and the same equal weights, four transport
# metrics: the discrete transport barycenter under the geometric, arithmetic, harmonic
# and (exact) logarithmic means on the 49-node USA graph, computed by Riemannian descent
...
# by the variance J. (The joint SOCP handles the first three means at N=32 in a few
# seconds each, but its quadrature-log program stalls in Clarabel for N ≥ 8 on this
# graph, which the library reports as an error rather than returning a stale iterate.)
#
# Also reports J = Σλᵢ W²(refᵢ, ν) for each mean (which orders like the means:
# harmonic ≥ geometric ≥ logarithmic ≥ arithmetic), the pairwise π-norm distances between
# the four barycenters, and the pairwise distance W(ref₁, ref₂) under each mean.
#
...
means = [("geometric", GeometricMean()), ("arithmetic", ArithmeticMean()),
         ("harmonic", HarmonicMean()), ("logarithmic", LogarithmicMean())]
norm_pi(v) = sqrt(sum(u .* v .^ 2))
...
println("\npairwise ‖ν_a − ν_b‖_π between the barycenters:")
for i in 1:4, j in i+1:4
    a, b = means[i][1], means[j][1]
    @printf("  %-18s vs %-18s %.4f\n", a, b, norm_pi(bars[a] .- bars[b]))
end

# ---- figure: row 1 the three references, row 2 the four barycenters, shared colour scale ----
...
order = sort(1:4; by=k -> Js[means[k][1]])          # ascending variance J = Σλᵢ W²(refᵢ, ν)
...
function panel!(pos, data, title)
    ...
    graphplot!(ax, g_plot; layout=(_) -> positions, node_color=data, node_size=14,
...
Colorbar(fig[1:2, 5], colormap=cmap, limits=(vmin, vmax), label="mass", labelsize=18, height=Relative(0.7))
